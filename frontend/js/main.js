...
async function createWebSocketWithRetry(endpoint, config, maxRetries = 3) {
    const { WS_BASE_URL, BACKUP_WS } = getApiConfig();
    const urls = [WS_BASE_URL, BACKUP_WS].filter(Boolean);
    
    for (let retry = 0; retry < maxRetries; retry++) {
        for (const baseUrl of urls) {
            try {
                console.log(`WebSocket 연결 시도 ${retry + 1}/${maxRetries}: ${baseUrl}`);
                
                const ws = new WebSocket(`${baseUrl}/ws/${endpoint}`);
                ...
                console.warn(`연결 실패 ${baseUrl}:`, error.message);
                continue;
            }
        }
...
async function startCrawling() {
    ...
    try {
        ...
        currentSocket = new WebSocket(`${WS_BASE_URL}/ws/${endpoint}`);

        currentSocket.onopen = () => {
            console.log('WebSocket 연결됨:', `${WS_BASE_URL}/ws/${endpoint}`);
    ...
    try {
        ...
        showTemporaryMessage('크롤링 중 오류가 발생했습니다.', 'error');
        hideProgress();
        isLoading = false;
        updateCrawlButton();
        currentSocket = null;
