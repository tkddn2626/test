...
async def unified_crawl_endpoint(websocket: WebSocket):
   ...
   try:
       ...
       # ✅ 번역 설정 디버깅 로그 추가
       logger.info(f"🔧 번역 설정 수신: translate={config.get('translate')}, skip_translation={config.get('skip_translation')}, target_languages={config.get('target_languages')}")
       ...
       if skip_translation or not translate or not target_languages:
           # 번역하지 않음 - 원제목만 사용
           logger.info(f"🚫 번역 건너뛰기: skip_translation={skip_translation}, translate={translate}, target_languages={target_languages}")
...
async def crawl_auto_socket(websocket: WebSocket):
   ...
   try:
       ...
       # ✅ 번역 설정 디버깅 로그 추가
       logger.info(f"🔧 레거시 번역 설정: translate={init_data.get('translate')}, skip_translation={init_data.get('skip_translation')}")
       ...
       # ✅ 번역 설정 확인 추가
       translate = init_data.get("translate", True)  # 기본값은 True (기존 동작 유지)
       skip_translation = init_data.get("skip_translation", False)
       ...
       # ✅ 수정된 번역 로직
       if skip_translation or not translate:
           # 번역하지 않음
           logger.info(f"🚫 레거시 번역 건너뛰기: skip_translation={skip_translation}, translate={translate}")
...
def root():
   return {
       ...
       "status": "running",
       "version": "2.0.1 (Translation Fixed)",
       "docs": "/docs",
...
if __name__ == "__main__":
   ...
   logger.info("🚀 PickPost v2.0.1 서버 시작 (번역 수정 버전)")
